
#ifndef DETAIL_DX_ADAPTER_H
#define DETAIL_DX_ADAPTER_H

#include <ren/renderer.h>
#include <ren/texture.h>
#include <mutex>

using namespace evo::ren;

namespace dx_adapter
{
	class image_data
	{
	public:
		image_data( std::vector<uint32_t>& v )
		{
			const auto root = make_offset( "panorama.dll", sig_construct_image );
			memset( data, 0, 0x2c );
			*reinterpret_cast< uintptr_t* >( uintptr_t( this ) ) = uintptr_t( v.data() );
			*reinterpret_cast< uint32_t* >( uintptr_t( this ) + 4 ) = sizeof( uint32_t ) * v.size();
			*reinterpret_cast< uintptr_t* >( uintptr_t( this ) + 0x24 ) = *reinterpret_cast< uintptr_t* >( root - 12 );
			*reinterpret_cast< uintptr_t* >( uintptr_t( this ) + 0x28 ) = *reinterpret_cast< uintptr_t* >( root - 4 );
		}

		bool load_svg( const uint8_t* img, const size_t s, uint32_t* w, uint32_t* h )
		{
			auto f = make_offset( "panorama.dll", sig_parse_svg );
			uint16_t magic[] = { 0x33, 0x34, 0x35, 0x5c, 0x37, 0x33, 0x30, 0x5c, 0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x5c, 0x70, 0x61, 0x6e, 0x6f, 0x72, 0x61, 0x6d, 0x61, 0x00 };
			auto scl = 1.f;
			auto rval = false;
			auto pad = data;

			__asm
			{
				mov edx, s
				lea ecx, magic
				push ecx
				push scl
				push h
				push w
				mov ecx, pad
				push ecx
				mov ecx, img
				call f
				add esp, 0x14
				mov rval, al
			}

			return rval;
		}

	private:
		uint8_t data[ 0x2c ];
	};

	struct icon_texture
	{
		std::shared_ptr<evo::ren::texture> texture;
		float width_to_height;
	};

	inline constexpr uint8_t esp_layer = 50;
	inline std::unordered_map<uint32_t, icon_texture> icon_textures = {};

	inline std::mutex draw_icon_mutex{};

	inline tsl::robin_map<uint32_t, std::string> icon_alias =
	{
		{ FNV1A( "taser" ), _w( "<svg id=\"svg\" version=\"1.1\" width=\"608\" height=\"689\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" ><g id=\"svgg\"><path id=\"path0\" d=\"M185.803 18.945 C 184.779 19.092,182.028 23.306,174.851 35.722 C 169.580 44.841,157.064 66.513,147.038 83.882 C 109.237 149.365,100.864 163.863,93.085 177.303 C 88.686 184.901,78.772 202.072,71.053 215.461 C 63.333 228.849,53.959 245.069,50.219 251.505 C 46.480 257.941,43.421 263.491,43.421 263.837 C 43.421 264.234,69.566 264.530,114.025 264.635 L 184.628 264.803 181.217 278.618 C 179.342 286.217,174.952 304.128,171.463 318.421 C 167.974 332.714,160.115 364.836,153.999 389.803 C 147.882 414.770,142.934 435.254,143.002 435.324 C 143.127 435.452,148.286 428.934,199.343 364.145 C 215.026 344.243,230.900 324.112,234.619 319.408 C 238.337 314.704,254.449 294.276,270.423 274.013 C 286.397 253.750,303.090 232.582,307.519 226.974 C 340.870 184.745,355.263 166.399,355.263 166.117 C 355.263 165.937,323.554 165.789,284.798 165.789 C 223.368 165.789,214.380 165.667,214.701 164.831 C 215.039 163.949,222.249 151.366,243.554 114.474 C 280.604 50.317,298.192 19.768,298.267 19.444 C 298.355 19.064,188.388 18.576,185.803 18.945 \" stroke=\"none\" fill=\"#ffffff\" fill-rule=\"evenodd\"></path></g></svg>" ) },
	};

	std::shared_ptr<evo::ren::texture> load_svg_to_texture( const uint8_t* buffer, size_t size, uint32_t w, uint32_t h );
	std::optional<icon_texture> icon_to_texture( const char* name, uint32_t target_height = 0 );
	std::optional<icon_texture> custom_icon_to_texture( const char* name, std::string data, uint32_t target_height = 0 );
	void weapon( evo::ren::layer& b, const vec2& pos, const float height, const color& clr, const char* name, char ah = 0, char av = 0, bool dropshadow = true );
	void icon( evo::ren::layer& b, const vec2& pos, const float height, const color& clr, const char* name, char ah = 0, char av = 0, bool dropshadow = true );
	void custom_icon( evo::ren::layer& b, const vec2& pos, const float height, const color& clr, const char* name, char ah = 0, char av = 0, bool dropshadow = true );
	float get_weapon_width( const float height, const char* name );
	float get_icon_width( const float height, const char* name );

	void swap_buffers();
	void clear_buffer();

	inline std::mutex mtx{};
	inline std::atomic_bool ready{};

	inline layer buf;
};

#endif // DETAIL_DX_ADAPTER_H
...
namespace util
{
	struct function_profiler_t
	{
		...
		function_profiler_t() : m_notify( nullptr )
		{
			m_start = std::chrono::steady_clock::now();
		}

		function_profiler_t( std::function<void( uint32_t )> notify ) : m_notify( notify )
		{
			m_start = std::chrono::steady_clock::now();
		}
	...
	struct function_profiler_average_t
	{
		...
		function_profiler_average_t( uint32_t calls_to_notify = 50 ) : m_calls_to_notify( calls_to_notify )
		{
			m_start = std::chrono::steady_clock::now();
		}
		...
		~function_profiler_average_t()
		{
			...
			if ( ++m_counter > 100 )
			{
				util::print_dev_console( true, Color::Lime(), "max: %d, min %d, avg: %f\n", m_max, m_min, m_average );
